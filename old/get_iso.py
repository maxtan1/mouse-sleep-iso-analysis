...
def main():
    os.makedirs('figures_iso_MSSV', exist_ok=True)
    ...
    data_dir = "/data/haoqisun/ShenShiqian_WangWei_project/MSSV_mice_sleep_staging_dataset_openneuro_ds006366"
    ...
    for sid in unique_sids:
        eeg_dir = os.path.join(data_dir, sid, "eeg")
        edf_paths_ = sorted(glob.glob(os.path.join(eeg_dir, "*_eeg.edf")))
        sids.extend([sid]*len(edf_paths_))
        edf_paths.extend(edf_paths_)
        annot_paths.extend([x.replace('_eeg.edf', '_events.tsv') for x in edf_paths_])
    df = pd.DataFrame(data={'SID':sids, 'EDFPath':edf_paths, 'AnnotPath':annot_paths})
    df.to_csv('mastersheet_MSSV.csv', index=False)
    ...
    df_res.to_csv('iso_results_all_subjects_bands_MSSV.csv', index=False)

    # save bout_band_powers_all_subjects_bands so that we can use it for cross-band correlation analysis
    with open('data_all_subjects_bands_MSSV.pickle', 'wb') as f:
        pickle.dump({
