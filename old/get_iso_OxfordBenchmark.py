...
def get_iso(edf_path, annot_path, band_lb, band_ub, band_name, to_plot=False, eeg_ch='EEG'):
    # load signal
...
def main():
    figure_dir = 'figures_iso_OxfordBenchmark'
    ...
    data_dir1 = "/data/haoqisun/ShenShiqian_WangWei_project/OxfordMouseBench/pilot/recordings"
    data_dir2 = "/data/haoqisun/ShenShiqian_WangWei_project/OxfordMouseBench/pilot/annotations"
    sids = sorted([x[:-4] for x in os.listdir(data_dir1)])
    edf_paths = [os.path.join(data_dir1, sid+'.edf') for sid in sids]
    annot_paths = [os.path.join(data_dir2, sid+'_consensus.hyp') for sid in sids]
    data_dir1 = "/data/haoqisun/ShenShiqian_WangWei_project/OxfordMouseBench/test/recordings"
    data_dir2 = "/data/haoqisun/ShenShiqian_WangWei_project/OxfordMouseBench/test/annotations"
    sids_ = sorted([x[:-4] for x in os.listdir(data_dir1)])
    sids.extend(sids_)
    edf_paths.extend([os.path.join(data_dir1, sid+'.edf') for sid in sids_])
    annot_paths.extend([os.path.join(data_dir2, sid+'_consensus_state_annotation.hyp') for sid in sids_])
    df = pd.DataFrame(data={'SID':sids, 'EDFPath':edf_paths, 'AnnotPath':annot_paths})
    df.to_csv('mastersheet_OxfordBenchmark.csv', index=False)
    ...
    for sid, edf_path, annot_path in tqdm(zip(sids, edf_paths, annot_paths), total=len(sids)):
        if '/test/' in edf_path:
            eeg_ch = 'Signal 3'
        ...
        for band_name in band_names:
            ...
            peak_freq, peak_psd, iso_bp, iso_bp_n, all_bout_band_powers, all_bout_phases, iso_freq, avg_iso_psd = \
                get_iso(edf_path, annot_path, band_lb, band_ub, band_name, to_plot=plot_path, eeg_ch=eeg_ch)
    ...
    df_res.to_csv('iso_results_all_subjects_bands_OxfordBenchmark.csv', index=False)

    # save bout_band_powers_all_subjects_bands so that we can use it for cross-band correlation analysis
    with open('data_all_subjects_bands_OxfordBenchmark.pickle', 'wb') as f:
        pickle.dump({
