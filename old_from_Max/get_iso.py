...
def get_iso(sid, edf_path, annot_path, band_lb, band_ub, band_name, to_plot=False):
    ...
    if len(EEG_index)==0:
        raise ValueError(f"{sid}:No EEG channel found")
    ...
    # for example, if sampling_frequency = 128Hz, when in 1 second, there are 128 sampling points.
    T = len(eeg) / fs
    ...
    #print(df_annot)
    sleep_stages = np.zeros(len(eeg))
    for i in range(len(df_annot)):
        # for each row in the annotation file, get its onset, duration, and stage
        onset=  df_annot.onset.iloc[i]
        duration = df_annot.duration.iloc[i]
        stage = df_annot.stage.iloc[i]
        start_index = int(round(onset*fs))
        end_index = int(round((onset+duration)*fs))
        # maybe due to mistake, start_index is <0 or end_index is >len(eeg)
        start_index = max(0,start_index)
        end_index = min(len(eeg),end_index)
        if start_index<end_index:
            sleep_stages[start_index:end_index] = stage
    ...
    minimum_size = int(round(minimum_duration*fs))
    start_end_indices_NREM = []
    ...
    """ this is Max's version, which is correct, but very slow
    i = 0
    while i < len(sleep_stages):
        if sleep_stages[i] == 2:
            j = i
            while j < len(sleep_stages) and sleep_stages[j] == 2:
                j += 1
            length = j - i
            if length >= minimum_size:
                start_end_indices_NREM.append((i, j))
            i = j
        else:
            i += 1
    """
    ...
    for k,l in groupby(sleep_stages):
        ...
        if k == 2:
            ...
            if length >= minimum_size:
                start_end_indices_NREM.append((start_index, end_index))
        counter +=len(l2)
    ...
    for start_index, end_index in start_end_indices_NREM:
        # for each NREM bout
        eeg_bout = eeg[start_index:end_index]  # eeg_bout.shape = (#sample points in this bout,)
        # segemnt eeg_bout to 4-second epochs, where each epoch has size `epoch_size`
        n_epochs = len(eeg_bout) // epoch_size 
        epochs = np.array([
            eeg_bout[e * epoch_size : (e + 1) * epoch_size]
        ...
        all_bout_band_powers.append(band_powers)
    ...
    if to_plot:
        ...
        plt.tight_layout()
        #plt.show()
        plt.savefig(f'figures_iso/iso_psd_{sid}_{band_name}.png', dpi=300, bbox_inches='tight')

    
    return peak_freq, peak_psd, iso_bp, iso_bp_n, all_bout_band_powers, iso_freq, avg_iso_psd



def main():
    os.makedirs('figures_iso', exist_ok=True)

    # loop over all EDF files
    # get the list of EDF files, and their corresponding annotation files, and their subject IDs, and save them in `sids`, `edf_paths`, and `annot_paths`
    data_dir = r"C:\Users\maxta\Downloads\ds006366"

    sids = [x for x in os.listdir(data_dir) if x.startswith('sub-')]
    edf_paths = [os.path.join(data_dir, f"{sid}/eeg/{sid}_task-sleep_run-1_eeg.edf") for sid in sids]
    annot_paths = [os.path.join(data_dir, f"{sid}/eeg/{sid}_task-sleep_run-1_events.tsv") for sid in sids]
    ...
    for sid, edf_path, annot_path in zip(sids, edf_paths, annot_paths):
        for band_name in band_names:
            print(f'{datetime.datetime.now()}: Processing {sid} - {band_name} band')
            band_lb, band_ub = band_name_to_freq[band_name]
            peak_freq, peak_psd, iso_bp, iso_bp_n, all_bout_band_powers, iso_freq, avg_iso_psd = get_iso(sid, edf_path, annot_path, band_lb, band_ub, band_name, to_plot=True)

            bout_band_powers_all_subjects_bands[(sid, band_name)] = all_bout_band_powers
            iso_freq_all_subjects_bands[(sid, band_name)] = iso_freq
            avg_iso_psd_all_subjects_bands[(sid, band_name)] = avg_iso_psd
    ...
    print(df_res)
    df_res.to_csv('iso_results_all_subjects_bands.csv', index=False)
    # save bout_band_powers_all_subjects_bands so that we can use it for cross-band correlation analysis
    with open('data_all_subjects_bands.pickle', 'wb') as f:
        pickle.dump({
